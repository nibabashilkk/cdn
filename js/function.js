...
var leonus = {
    ...
    randomLink: () => {
        ...
        if (e) {
            ...
            leonus.saveData("links", e.link_list), leonus.randomLink()
        }))
    },
    share: () => {
    ...
    runtime: () => {
        ...
        let n = new Date("2021/10/15 00:00:00").getTime(),
            t = (new Date).getTime(),
    ...
    randomPost: () => {
        ...
        if (e)
            for (;;) {
                ...
                if (n != location.pathname) return void pjax.loadUrl(n)
            }
    ...
    comCount: () => {
        function e(e) {
            try {
                document.querySelectorAll(".card_comment").forEach((n => {
                    n.innerHTML = e
                })), document.querySelector(".card-info .card_comment").parentNode.title = "累计评论数：" + e
            } catch (e) {}
        }
        if (document.querySelector(".card_comment")) {
            let n = leonus.loadData("comCount", 5);
            n ? e(n) : fetch("https://api.leonus.cn/comCount").then((e => e.text())).then((n => {
                e(n), leonus.saveData("comCount", n)
            })).catch()
        }
    },
    winbox: "",
    ...
    switchLine: () => {
        if ("b.leonus.cn" == location.host && (document.querySelector(".announcement_content").innerHTML = '您当前使用的是Vercel线路，欢迎使用主线路访问本站：<a style="color: var(--leonus-purple);" href="https://blog.leonus.cn/">blog.leonus.cn</a>', document.querySelectorAll(".menus_item:last-child ul li:first-child a").forEach((e => {
                e.href = "https://blog.leonus.cn/"
            }))), leonus.loadData("line", 60)) return;
        let e = location.host;
        document.referrer.match(/b.leonus.cn/i) && "blog.leonus.cn" == e && setTimeout((() => {
            btf.snackbarShow("欢迎使用主线路访问本站。"), leonus.saveData("line", 1)
        }), 500), document.referrer.match(/blog.leonus.cn/i) && "b.leonus.cn" == e && setTimeout((() => {
            btf.snackbarShow("欢迎使用Vercel线路访问本站。"), leonus.saveData("line", 1)
        }), 500)
    },
    searchSize: () => {
        if (document.querySelector(".algolia-poweredBy").addEventListener("DOMNodeInserted", (() => {
                document.querySelector("svg.ais-PoweredBy-logo").setAttribute("viewBox", "290 0 290 64")
    ...
    indexTalk: () => {
        ...
        function e(e) {
            let n = "";
            e.forEach(((e, t) => {
                n += `<li class="item item-${t+1}">${e}</li>`
            }));
            let t = document.querySelector("#bber-talk .talk-list");
            t.innerHTML = n, leonus.talkTime = setInterval((() => {
                t.appendChild(t.children[0])
            }), 3e3)
        }
        let n = leonus.loadData("talk", 10);
        n ? e(n) : fetch("https://m.leonus.cn/api/memo?creatorId=1&tag=说说&limit=10").then((e => e.json())).then((n => {
            e(n = function (e) {
                let n = [];
                return e.forEach((e => {
                    n.push(e.content.replace(/#(.*?)\s/g, "").replace(/\{(.*?)\}/g, "").replace(/\!\[(.*?)\]\((.*?)\)/g, '<i class="fa-solid fa-image"></i>').replace(/(?<!!)\[(.*?)\]\((.*?)\)/g, '<i class="fa-solid fa-link"></i>'))
                })), n
            }(n.data)), leonus.saveData("talk", n)
        }))
    },
    ...
    newYearBg: () => {
        ...
        e && (e.style.backgroundImage = `url(https://cdn.leonus.cn/img/tunian${leonus.newYearBgNum}.webp)`, leonus.newYearBgTimer = setInterval((() => {
            e.style.backgroundImage = `url(https://cdn.leonus.cn/img/tunian${3==leonus.newYearBgNum?leonus.newYearBgNum=1:leonus.newYearBgNum+=1}.webp)`
        }), 1e4))
    },
    swiper: () => {
    ...
    photos: () => {
        fetch("https://m.leonus.cn/api/memo?creatorId=1&tag=相册").then((e => e.json())).then((e => {
            ...
            })), document.querySelector(".gallery-photos.page").innerHTML = n, imgStatus.watch(".photo-img", (() => {
